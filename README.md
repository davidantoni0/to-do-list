...
5- O sistema atualmente é fortemente focado nas permissões de administrador, portanto, certos elementos estarão bloqueados à usuarios. ao executar o comando post para criação de usuarios, deve-se alterar o cargo 'role' de 'user' para 'admin' diretamente no banco de dados. Essa medida foi criada afim de evitar a criação de admins diretamente pela rota de criação. O cargo de administradores só poderá ser gerado na rota updateUsers caso o token da requisição for de outro administrador.